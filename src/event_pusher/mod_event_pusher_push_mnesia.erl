...
-spec get_publish_services(User :: jid:jid()) ->
                                  {ok, [mod_event_pusher_push:publish_service()]} |
                                                 {error, Reason :: term()}.
get_publish_services(User) ->
    case safe_read(key(User)) of
        {ok, Records} ->
            {ok, [{Resource, PubsubJID, Node, Forms} ||
                #push_subscription{resource = Resource,
...
make_record(UserJID, PubsubJID, Node, Form) ->
    #push_subscription{
       user_jid = key(jid:to_bare(UserJID)),
       resource = UserJID#jid.resource,
       pubsub_jid = PubsubJID,
       pubsub_node = Node,
       form = Form
      }.
